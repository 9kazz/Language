...
TreeErr_t Graphic_Dump_Tree(const TreeNode_t* root, const char* dump_info, const char* img_dir) {
    assert(root);
    assert(img_dir);
    ...
    char graphviz_src_str [MAX_STR_LEN] = {0};
    char create_image_CMD [MAX_STR_LEN] = {0};

    SAFE_SNPRINTF(image_name_str,   MAX_STR_LEN, "%s/ReverseTree_%d.png",  img_dir, dump_num); 
    SAFE_SNPRINTF(graphviz_src_str, MAX_STR_LEN, "%s/ReverseGraphSrc.txt", img_dir);

    SAFE_FOPEN(graphviz_file, graphviz_src_str, "w")

    Print_Tree_to_Graphviz(root, graphviz_file);

    fclose(graphviz_file);

    SAFE_SNPRINTF(create_image_CMD, MAX_STR_LEN, "dot -Tpng %s -o %s", graphviz_src_str, image_name_str);
